...
public class HsActivity_DJ_HsClsyjl extends HsActivity_DJ
{
	private IHsLabelValue mCl;
	
	private UcDateBox ucJlrq;

	private UcTextBox ucJlzy;

	public HsActivity_DJ_HsClsyjl()
	{
		super();
		
		this.inDJParams = new DJParams(false, true, false);
		
		this.mAnnexClass = HSOADjlx.HSCLSYJL;
	}
	...
	protected void initInComingVariable(Bundle bundle) throws Exception
	{
		...
		mCl = (IHsLabelValue) bundle.getSerializable("Cl");
	}
	...
	protected void initComponent(Bundle savedInstanceState) throws Exception
	{
		...
		this.setTitleSaved(HSOADjlx.HS车辆生涯记录);
		...
		this.ucJlzy.setCName("备注");
		this.ucJlzy.setAllowEmpty(true);
	...
	{
		return ((HSOAWebService) application.getWebService()).updateHsClsyjl(
							application.getLoginData().getProgressId(),
							this.getDJId(),
							this.mCl.getValue("ClId", "-1").toString(),
	...
	public void actionAfterWSReturnData(String funcname, Serializable data)
			throws Exception
	{
		if (funcname.equals("UpdateHsClsyjl"))
		{
	...
	private class HsClsyjlFragment extends HsFragment_ZD_Main
	{
