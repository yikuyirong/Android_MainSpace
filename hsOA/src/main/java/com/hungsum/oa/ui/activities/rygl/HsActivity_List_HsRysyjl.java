...
public class HsActivity_List_HsRysyjl extends HsActivity_List_DJ
{
	public HsActivity_List_HsRysyjl()
	{
		super();
	...
	{
		return ((HSOAWebService)application.getWebService()).showHsRysyjls(
				application.getLoginData().getProgressId(),
				mNecessaryData.getValue("RyId","-1").toString(),
				getBeginDateValue(),
				getEndDateValue(),
				getSwitchValues());
	}
	...
	{
		Intent intent = new Intent(this,HsActivity_DJ_HsRysyjl.class);
		intent.putExtra("Ry", this.mNecessaryData);
	...
	protected void modifyItem(IHsLabelValue item) throws Exception
	{
		...
		intent.putExtra("Ry", mNecessaryData);
	...
	public void actionAfterWSReturnData(String funcname, Serializable data)
			throws Exception
	{
		if(funcname.equals("DoDatas_Delete_HsRysyjl") ||
		   funcname.equals("DoDatas_Submit_HsRysyjl") ||
		   funcname.equals("DoDatas_UnSubmit_HsRysyjl"))
		{
			ShowInformation(data.toString());
			callRetrieveOnOtherThread(false);
		}else if(funcname.equals("ShowHsRydas"))
		{
