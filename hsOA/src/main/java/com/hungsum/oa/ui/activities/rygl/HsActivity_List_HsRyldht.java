...
public class HsActivity_List_HsRyldht extends HsActivity_List_DJ
{
	public HsActivity_List_HsRyldht()
	{
		super();
			...
			ContextMenuInfo menuInfo)
	{
		...
		String htzt = item.getValue("Htzt", "-1").toString();

		menu.addSubMenu(0,R.string.str_new, 0,"新增").setIcon(R.drawable.content_new);
		if(htzt.equals("0")) //未提交
		{
			menu.addSubMenu(0,R.string.str_modify, 0, "修改");
			menu.addSubMenu(0,R.string.str_delete, 1, "删除").setIcon(R.drawable.content_remove);
			menu.addSubMenu(0,R.string.str_submit, 2, "提交");
		}else {
			menu.addSubMenu(0,R.string.str_modify, 0, "查看");
			menu.addSubMenu(0,R.string.str_unsubmit, 1, "取消提交");
		}
	...
	{
		return ((HSOAWebService)application.getWebService()).showHsRyldhts(
	...
	{
		Intent intent = new Intent(this,HsActivity_DJ_HsRyldht.class);
	...
	protected void modifyItem(IHsLabelValue item) throws Exception
	{
		...
		if(item.getValue("Htzt", "0").equals("1"))
		{
	...
	public void actionAfterWSReturnData(String funcname, Serializable data)
			throws Exception
	{
		if(funcname.equals("DoDatas_Delete_HsRyldht") ||
		   funcname.equals("DoDatas_Submit_HsRyldht") ||
		   funcname.equals("DoDatas_UnSubmit_HsRyldht"))
		{
